...
def divide_track_stop(data):
    ...
    (grouped_data['max_x'] - grouped_data['min_x'] < MAX_DISTANCE_FOR_TRACK_STOP_DIVISION_X) &
    (grouped_data['max_y'] - grouped_data['min_y'] < MAX_DISTANCE_FOR_TRACK_STOP_DIVISION_Y)
    ).astype(int)
    geo_data = pd.merge(geo_data, grouped_data[['mapped_veh_id', 'date', 'track_or_stop']], on=['mapped_veh_id', 'date'], how='left')
