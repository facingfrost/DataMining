...
def generateModel(X_train, y_train):
    ...
    while (pValues[0] > 0.05) | (varParam[0] > 0):
        if pValues[0] > 0.05:
            non_significant_variables.append(pValues.index[0])
        elif varParam[0] > 0:
            non_significant_variables.append(varParam.index[0])
        else:
            print("Error")
        print(non_significant_variables)
        model = sm.Logit(y_train, X_train.loc[:, sorted(
            list(set(X_train.columns) - set(non_significant_variables)))])
        results = model.fit()
        pValues = results.pvalues.sort_values(ascending=False)[:1]
        varParam = results.params.sort_values(ascending=False)[:1]
        print(pValues[0])
        print(varParam.index[0])
    return non_significant_variables, results
...
def pred_proba(results, non_significant_variables, X_train, X_test):
    '''predict data'''
    prob_threshold = 0.5
    train_pred_proba = results.predict(X_train.loc[:, sorted(
        list(set(X_train.columns) - set(non_significant_variables)))])
    test_pred_proba = results.predict(X_test.loc[:, sorted(
        list(set(X_train.columns) - set(non_significant_variables)))])
...
def main():
    ...
    predict_vars = set(predict_vars) - \
        {'RS_E_OilPress_PC1_WOE_OB', 'RS_E_WatTemp_PC1_WOE_OB', }
    # df filtered for model
    df_model = df.loc[(df['period'] >= '2023-04-01') &
                      (df['target'].isin([0, 1])), predict_vars + ['target']]
    # split data
    X_train, X_test, y_train, y_test = split_train_test(
        df_model, predict_vars, test_size=0.3, target='target')
    # add constant
    X_train = sm.add_constant(X_train)
    X_test = sm.add_constant(X_test)
    # model
    non_significant_variables, results = generateModel(X_train, y_train)
    print(results)
