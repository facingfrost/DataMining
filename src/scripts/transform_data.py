...
def formatTablewoeiv(df, flagTotal=False):
    ...
    if flagTotal:
        df.loc[df['Bin'].fillna('') == '', ['Bin', 'CLAVE']] = [
            'Total', '99.- Total']
    else:
...
def woeiv_table(df, target, varNumeric, dictUserSplits):
    ...
    for col in varNumeric:
        ...
        optb = OptimalBinning(
            ...
            user_splits=dictUserSplits.get(
                col,
                None))
        optb.fit(x, y)
        ...
        if not dfWOEIV.empty:
            dfWOEIV = dfWOEIV.loc[dfWOEIV['VARIABLE']
...
def transform_datawoe(df, dfWOEIV, dictOB, varModel, dictSpecialCodes={}):
    ...
    for variable in varModel:
        ...
        try:
            valorNuloWOE = dfWOEIV.loc[(dfWOEIV['VARIABLE'] == variable) & (
                dfWOEIV['Bin'] == 'Missing'), 'WoE'].values[0]
            df.loc[df[variable].isnull(), variable + '_WOE_OB'] = valorNuloWOE
        except BaseException:
            print(variable + ':No Nulls')
        try:
            valorEspecialWOE = dfWOEIV.loc[(dfWOEIV['VARIABLE'] == variable) & (
                dfWOEIV['Bin'] == 'Special'), 'WoE'].values[0]
            df.loc[df[variable].isin(
                dictSpecialCodes[variable]), variable + '_WOE_OB'] = valorEspecialWOE
        except BaseException:
